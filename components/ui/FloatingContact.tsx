...
export default function FloatingContact() {
  ...
  return (
    <div
      className="fixed right-6 bottom-8 z-50 flex flex-col items-center"
      aria-label="Liên hệ nhanh"
    >
      {/* Social options stack (positioned absolutely above the main toggle button) */}
      <div
        className={`absolute bottom-16 flex flex-col items-center gap-3.5 transition-all duration-300 origin-bottom ${
          isOpen
      ...
      <button
        ...
        className="w-13 h-13 rounded-full bg-secondary text-on-secondary flex items-center justify-center shadow-lg hover:shadow-2xl hover:scale-105 active:scale-95 transition-all duration-300 cursor-pointer relative z-50"
        aria-label={isOpen ? "Đóng danh sách liên hệ" : "Mở danh sách liên hệ"}
      >
        <span
          className={`material-symbols-outlined transition-transform duration-300 relative z-10 ${
            isOpen ? "rotate-90" : ""
          }`}
          style={{ fontSize: 24 }}
        >
        ...
        {!isOpen && (
          <span className="absolute inset-0 rounded-full bg-secondary opacity-30 animate-ping z-0" />
        )}
      </button>
    </div>
  );
